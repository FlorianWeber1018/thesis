...
Die erste Tabelle hat die Bezeichnung \eigenName{Abteilung} (\refFig{fig:examplesSQLStructure:abteilung}) mit den Spalten \eigenName{ID}, \eigenName{Bezeichnung} sowie \eigenName{Standort}.
Die Spalte \eigenName{ID} ist als Primary-Key deklariert.
Die zweite Tabelle \eigenName{Mitarbeiter} (\refFig{fig:examplesSQLStructure:mitarbeiter}) enthält die Spalten 
...
\end{figure}
Die Antwort des Servers ist in Abbildung \refFig{fig:examplesSQLStructure:result} zu sehen.
Bei \refFig{fig:examplesSQLStructure:abteilung} und \refFig{fig:examplesSQLStructure:mitarbeiter} sieht man die Datensätze der beiden Quelltabellen des Querys.
