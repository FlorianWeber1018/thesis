...
\end{figure}
Wie in Abbildung \ref{fig:exampleSQLStructure} zu sehen, besteht die Datenbank \eigenName{Personal} aus zwei Tabellen.
Die erste Tabelle hat die Bezeichnung (\eigenName{Abteilung}) mit den Spalten \eigenName{ID}, \eigenName{Bezeichnung} sowie Standort.
Die Spalte \eigenName{ID} ist als Primary-Key deklariert.
Die zweite Tabelle (\eigenName{Mitarbeiter}) enthält die Spalten 
...
wird ein Fremschlüssel definiert, der von der Spalte \eigenName{AbteilungID} der Tabelle \eigenName{Mitarbeiter} auf die Tabelle \eigenName{Abteilung}, 
