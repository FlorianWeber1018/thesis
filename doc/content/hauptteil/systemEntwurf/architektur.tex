...
Zusätzlich speichert es die \emph{pageID}, die angibt auf welche Seite aktuell dargestellt wird, eine Variable \eigenName{authenticated}, 
als Indikator ob die Websocket Verbindung bereits authentifiziert ist, sowie ein \eigenName{pageStruct} Objekt, dass alle weiteren Daten zur Aktuellen Seite beinhaltet.
...
Bei der Intitaliesierung des State Objekts ist das \eigenName{pageStruct} Objekt immer \emph{null}, 
...
Diese Objekte wurden in dem Klassendiagramm als \eigenName{page} Klasse gekenntzeichnet und beinhalten die ID der Seite (\eigenName{id}), 
...
Zusätzlich einhält das \eigenName{pageStruct} Objekt beliebig viele GuiElemente (\eigenName{guiElements}) diese werden allerdings nicht als Array gespeichert, sondern als Objekt, indem jedes Attribut des Objekts ein \eigenName{guiElement} Objekt enthält.
...
einer Beschreibung (\eigenName{description}), sowie soviele DataNodes und ParamNodes wie die Vorlage der Backend Datenstruktur (\refFig{img:erd}), des entsprechenden \eigenName{GuiElementTypes} vorgibt.
...
Jedes \eigenName{node} Objekt hat unabhängig ob eine \eigenName{dataNode} oder eine \eigenName{paramNode} ist, 
...
Der Typ einer Node ist immer ein Typ, entsprechend der expliziten Angabe des Typs (Attribut \eigenName{type}), eines einzelnen \eigenName{node} Objekts.